...
class TestTap0 < Test::Unit::TestCase
  def test_run
    ...
    output = StringIO.new
    runner = Test::Unit::UI::Tap::OldTestRunner.new(test_case.suite,
                                                 :output => output)
...
#      #{File.expand_path(__FILE__+'/../..')}/lib/test/unit/ui/tap/testrunner.rb:134:in `start'
...
EOR
    assert_true(result.passed?)  # think this is incorrect
  end
end
